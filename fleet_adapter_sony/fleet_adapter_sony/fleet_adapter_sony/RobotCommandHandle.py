...
from rmf_adapter.robot_update_handle import Tier, IssueTicket, Commission
...
# robo_path_msgs は環境によって未導入のことがあるため optional とする
try:
    from robo_path_msgs.msg import RoboPath, Pose  # type: ignore
except Exception:  # pragma: no cover
    RoboPath = None  # type: ignore
...
class RobotCommandHandle(adpt.RobotCommandHandle):
    def __init__(self,
        ...
        start_wp = self.graph.get_waypoint(start.waypoint)
        self.now_waypoint = start_wp.waypoint_name
        self.node.get_logger().info(
            f"The robot is starting at: [{self.position[0]:.2f}, "
            f"{self.position[1]:.2f}, {self.position[2]:.2f}, "
            f"waypoint name: {self.now_waypoint}]")
        ...
        def _follow_path():
            ...
            while (
                ...
                if self.state == RobotState.IDLE:
                    ...
                    # NOTE: transforms are in meters + radians.
                    # api.navigate() is expected to accept [x, y, theta] in the same unit.
                    waypoint_name = None
                    try:
                        if self.target_waypoint.graph_index is not None:
                            waypoint_name = self.graph.get_waypoint(
                                self.target_waypoint.graph_index).waypoint_name
                    except Exception:
                        waypoint_name = None

                    # angle_rad は RMF の座標系（map_name の座標系）での姿勢。
                    # Sony のように「目的地タグ + angle(rad)」で指令するAPIではこちらを優先して使える。
                    angle_rad = None
                    try:
                        angle_rad = float(target_pose[2])
                    except Exception:
                        angle_rad = None
                        ...
                        if (self.api.navigation_completed(self.name)):
                            ...
                            if lane is not None:
                                ...
                                if self.target_waypoint.graph_index is not \
                                    ...
                                    self.on_waypoint = None
                        # NOTE: If the vendor API cannot report remaining duration,
                        # estimate it from distance/speed in your RobotClientAPI.
    ...
    def dock(
        ...
        def _dock():
            ...
            with self._lock:
                ...
                self.on_lane = None
            self.sleep_for(0.1)
            # NOTE: docking completion is tracked by api.docking_completed().
    ...
    def get_position(self):
        ...
        if position is not None:
            ...
            theta = math.radians(position[2]) - \
                self.transforms['orientation_offset']
            # NOTE: transforms are expected to yield meters + radians.
